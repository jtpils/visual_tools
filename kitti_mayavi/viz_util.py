...
def draw_lidar(pc, color=None, fig=None, bgcolor=(0,0,0), pts_scale=1, pts_mode='point', pts_color=None):
    ''' Draw lidar points with square region and fov.
    Args:
    ...
    mlab.plot3d([0, axes[0,0]], [0, axes[0,1]], [0, axes[0,2]], color=(1,0,0), tube_radius=None, figure=fig)  # red for x
    mlab.plot3d([0, axes[1,0]], [0, axes[1,1]], [0, axes[1,2]], color=(0,1,0), tube_radius=None, figure=fig)  # green for y
    mlab.plot3d([0, axes[2,0]], [0, axes[2,1]], [0, axes[2,2]], color=(0,0,1), tube_radius=None, figure=fig)  # blue for z
...
def draw_gt_boxes3d(gt_boxes3d, fig, color=(1,1,1), line_width=1, draw_text=True, text_scale=(1,1,1), color_list=None):
    ''' Draw 3D bounding boxes in pointcloud fig
    Args:
...
def save_pcfig(pc, box, path):
    """
    input:
        pc: (n, 3)
        box: (b, 8, 3)
    return:
        save fig in the path
    """
    fig = draw_lidar(pc)
    fig = draw_gt_boxes3d(box, fig)
    mlab.savefig(path+'/test.jpg', figure=fig)


if __name__=='__main__':
    import os
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(BASE_DIR, 'kitti_sample_scan.txt')
    ...
    fig = draw_lidar(pc)
    box = np.array([[[0, 0,0],[0,10,0],[10,10,0],[10,0,0],[0,0,10],[0,10,10],[10,10,10],[10,0,10]]])
    print(box.shape)  # (1, 8, 3)
    fig = draw_gt_boxes3d(box, fig)
